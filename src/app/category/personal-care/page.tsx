...
import { Package, Sparkles } from 'lucide-react'; // Using Sparkles as a representative icon

export default function PersonalCareProductsPage() {
  const [personalCareProducts, setPersonalCareProducts] = useState<Product[]>([]);
  ...
  useEffect(() => {
    ...
    const personalCareQuery = query(productsCollectionRef, where('category', '==', categoryName));

    const unsubscribe = onSnapshot(personalCareQuery, (snapshot: QuerySnapshot<DocumentData>) => {
      const productsData = snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
      } as Product));
      setPersonalCareProducts(productsData);
      setIsLoading(false);
    }, (err: any) => {
  ...
  return (
    <div className="space-y-6"> {/* Reduced overall vertical spacing */}
      <section className="text-center py-6 bg-gradient-to-r from-blue-100 via-indigo-50 to-background rounded-xl shadow-md"> {/* Reduced vertical padding */}
        <Sparkles size={36} className="mx-auto text-blue-500 mb-3" /> {/* Reduced icon size and margin */}
        ...
        <p className="text-sm text-muted-foreground mt-2"> {/* Reduced paragraph text size */}
          Essential products for your daily personal care routine.
        </p>
      </section>
