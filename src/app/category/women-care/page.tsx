...
import { Package, HeartHandshake } from 'lucide-react'; // Using HeartHandshake as a representative icon

export default function WomenCareProductsPage() {
  const [womenCareProducts, setWomenCareProducts] = useState<Product[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const categoryName = "Women Care";

  useEffect(() => {
    setIsLoading(true);
    setError(null);
    const productsCollectionRef = collection(db, 'products');
    const womenCareQuery = query(productsCollectionRef, where('category', '==', categoryName));

    const unsubscribe = onSnapshot(womenCareQuery, (snapshot: QuerySnapshot<DocumentData>) => {
      const productsData = snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
      } as Product));
      setWomenCareProducts(productsData);
      setIsLoading(false);
    }, (err: any) => {
  ...
  return (
    <div className="space-y-6"> {/* Reduced overall vertical spacing */}
      <section className="text-center py-6 bg-gradient-to-r from-pink-100 via-purple-50 to-background rounded-xl shadow-md"> {/* Reduced vertical padding */}
        <HeartHandshake size={36} className="mx-auto text-pink-500 mb-3" /> {/* Reduced icon size and margin */}
        ...
        <p className="text-sm text-muted-foreground mt-2"> {/* Reduced paragraph text size */}
          Dedicated products for women's health and wellness.
        </p>
      </section>
